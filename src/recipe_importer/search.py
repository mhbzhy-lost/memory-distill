...
def _score(record: dict, query: str) -> int:
    ...
    fields = (
        ...
        + record["fingerprints"]
        + record["symptoms"]
        + [record["failure_class"], record["status"]]
    )
    return sum(1 for value in fields if normalized_query in value.lower() or value.lower() in normalized_query)
...
def get_recipe(paths: KbPaths, recipe_id: str) -> Recipe:
    data = read_json(paths.index_path)
